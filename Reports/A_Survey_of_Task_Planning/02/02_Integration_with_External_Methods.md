...
- **LLMの課題**
  ...
  - 暗黙的な知識に依存し、明示的な制約や物理法則の厳密な扱いに課題がある。
- **従来手法の課題**
  - PDDLのような標準言語は、複雑な現実世界のタスクにおけるモデリング能力と表現力に限界がある。
  - 自然言語のような曖昧な要求を直接解釈できず、厳密な形式化が必要となる。
...
3. **他分野理論との融合**
   認知科学、ゲーム理論、神経科学などの知見を取入れ、より高度で人間らしいプランニング能力を獲得する。

---

## 1. 古典的計画との統合: 構造と形式言語の力を活用する

### Key Concept
PDDL (Planning Domain Definition Language) を共通言語として、LLMの柔軟な言語理解能力と古典的プランナーの厳密な計画生成能力を橋渡しする。



---

## LLMが担う多様な役割: 翻訳者からプランナー、検証者まで

- **Role 1: 翻訳者 (Translator)**
  自然言語のタスク記述をPDDLや他の論理形式に変換。
  - **注目事例**: TWOSTEP - 目標分解を組み合わせ、マルチエージェントプランニングの計画時間を短縮。
- **Role 2: プランナー (Planner)**
  PDDLを直接解釈してプランを生成、あるいは自然言語からの変換とプランニングの両方を実行。
- **Role 3: ヒューリスティクス・検証者 (Heuristics/Verifier)**
  探索プロセス改善のためのヒューリスティクスを提供。外部検証者によるプランのフィードバック活用。

> Valmeekam et al. の発見: LLMが自律的に実行可能なプランを生成する能力は限定的だが、他プランナーの探索を助ける「ヒューリスティクス」や「外部検証者」と連携することで真価を発揮する。
...
- **エラーからの学習 (LASP)**
  実行エラー時、LLMが観測に基づき原因を診断。環境との対話を通じ、不完全な知識を持つオープンワールド問題に対応。
- **フィードバックによる述語学習 (InterPreT)**
  PDDLの基本要素「述語」を、人間の自然言語フィードバックに基づき、LLMが反復的に修正・学習する。
- **アフォーダンスに基づく計画 (AutoGPT+P)**
  オブジェクト検出とLLMによるマッピングを組み合わせ、物理環境における行動の可能性（アフォーダンス）から計画ドメインを導出（掴む、注ぐ、移動など）。



---

## 2. 強化学習との統合: フィードバックループによる最適化

### Key Concept
LLMが持つ汎用的な世界知識・多タスク学習能力と、強化学習の強力な意思決定能力を組み合わせ、双方の長所を最大限に引き出す。

---

## MCTSの強化: LLMを世界モデルおよび推論エージェントとして活用

プランニング問題はマルコフ決定過程 (MDP) として定式化され、MCTSはその有力な解法である。

- **世界モデル (World Model)**: LLMが誘導する世界モデルがMCTSに常識的な事前信念を提供し、探索効率を向上させる (RAP)。
- **価値関数学習 (Value Function Learning)**: AlphaZeroに着想を得て価値関数をファインチューニングし、ロールアウトプロセスを簡略化・安定化させる。
- **In-Context学習・計画**: MDPのアクター・クリティック更新を模倣し、事後理解を更新しながら最適な軌道を生成する (RAFA)。
- **探索と活用の高速化**: MCTSを核とし、追加の報酬層を組み込むことでオフラインログ活用や高速な解決を実現 (REX)。
...
1. **人間からのフィードバック (RLHF)**
   標準的手法だが、高品質な人間の嗜好ラベル収集が重大なボトルネックとなる。
2. **AIからのフィードバック (RLAIF)**
   人間の代わりにLLMが嗜好ラベルを付与。スケーラビリティ問題を解決し、大規模な最適化を可能にする。
3. **環境からのフィードバック (RLEF)**
   エージェントが環境と相互作用する中で得られる直接的なフィードバックを活用。
   - **Retroformer**: 環境フィードバックを用いてプロンプトを改善。
   - **Octopus**: 環境フィードバック情報で報酬モデルをファインチューニング。

---

## 3. 他分野との融合: 新たな視点による能力の拡張

### Key Concept
タスクプランニングは本質的に学際的な課題であり、認知心理学、ゲーム理論、神経科学などの理論を統合することで、より効果的で革新的なアプローチが生まれる。
...
- **心の理論 (Theory of Mind)**
  他者の精神状態（思考、信念、欲望）を理解し、行動を予測する能力。
  - **K-level reasoning**: 相手の視点を再帰的に採用し、予測精度を向上。
  - **Suspicion-agent**: GPT-4に心の理論を付与し、不完全情報ゲームにおける相手の思考を予測。
- **ゲーム理論 (Game Theory)**
  対立・協調状況下における最適な意思決定の研究。
  - **Alympics**: LLMエージェントを用いて人間行動をシミュレートするプラットフォーム。
  - **GAMA(γ)-Bench**: マルチエージェント環境におけるゲーミング能力の評価フレームワーク。

> 発見: GPT-4でさえ、欲望、信念、行動の合理性において人間とは大きな違いを示す。
...
- **神経科学 (Neuroscience)**
  人間の脳機能（前頭前野）や二重過程理論（速い思考と遅い思考）にヒントを得る。
  - 複数のGPT-4モジュールで前頭前野機能を模倣し多段階推論を強化。
  - **速い思考（ワンストップ推論）** と **遅い思考（ToT）** を統合した階層的計画アルゴリズムの提案。
- **グラフ理論 (Graph Theory)**
  タスクプランニングをグラフ上の経路選択問題（ノード＝サブタスク、エッジ＝依存関係）として定式化。
  - **Wu et al.**: LLMが要求を分解し、グラフニューラルネットワーク (GNN) が関連サブタスクを検索。GNNの出力をLLMのトークンとして利用する唯一の研究。
...
| 統合相手 | 相乗効果 |
| :--- | :--- |
| **古典的計画** | LLMが曖昧さを処理し、古典的プランナーが構造と実行可能性を保証。 |
| **強化学習** | LLMが報酬信号や世界モデルを提供し、RLが意思決定を最適化。 |
| **他分野理論** | 新たな視点とツールを提供し、研究の革新的な道を切り拓く。 |

**💡 スタンドアロンなLLMや古典的なAIプランニングシステムの根源的な限界を克服し、より堅牢で柔軟、かつ効果的なタスクプランニングを実現する。**
...
1. **フィードバックのバランス**
   人間による嗜好ラベルの希少性と、AIフィードバックによる価値整合 (Value Alignment) のバランス調整。
2. **スケーラビリティと汎化**
   LLMと外部モデル間のより深い相互作用を探求し、スケーラビリティと汎化の問題に対処することが現在の研究の焦点。
3. **倫理とデータ品質**
   倫理的配慮、高品質なデータとフィードバックメカニズムの確保は、実世界適用のための重要要素。
...
- **より効率的でスケーラブルな統合フレームワークの開発**
  動的で複雑な性質に対応できる柔軟なアーキテクチャの構築。新しい手法を容易に組み込める設計。
- **より良い評価指標の確立**
  実世界の制約や動的な性質を考慮した評価指標を開発し、統合アプローチの有効性と実用性をより正確に評価する。
